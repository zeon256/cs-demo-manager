...
type GenerateVideoWithFFmpegSettings = {
	...
	hudOverlayType: "file" | "url";
	hudOverlayFilePath: string;
...
export async function generateVideoWithFFmpeg(
	settings: GenerateVideoWithFFmpegSettings,
	signal: AbortSignal,
) {
	const { tgaFiles, wavFilePath, videoFilePath } =
		await getSequenceRawFiles(settings);
	...
	const hudFileExists =
		hudOverlayEnabled && hudOverlayType === "file" && hudOverlayFilePath !== ""
			? await fs.pathExists(hudOverlayFilePath)
			: false;

	const hudUrlFramesExists = hudOverlayEnabled && hudOverlayType === "url";
	const hudOverlayActive = hudFileExists || hudUrlFramesExists;
	...
	const outputPath = getSequenceOutputFilePath(
		outputFolderPath,
		sequence,
		videoContainer,
	);
	if (
		...
		if (hudFileExists) {
			args.push(`-i "${hudOverlayFilePath}"`);
		}

		if (hudFileExists) {
			args.push(
				`-filter_complex "[${hudInIndex}:v]scale=${width}:${height}[hud];[0:v][hud]overlay=0:0[outv]"`,
			);
		}
	...
	if (recordingSystem === RecordingSystem.HLAE) {
		...
		rawFilesPathPattern = path.join(
			...
			game === Game.CSGO
				? `${sequenceName}%04d.tga`
				: `${sequenceName}%08d.tga`,
		);
	}

	let hudOverlayFinalPath = hudOverlayFilePath;
	if (hudOverlayType === "url") {
		if (recordingSystem === RecordingSystem.HLAE) {
			hudOverlayFinalPath = path.join(
				sequenceRawFilesFolderPath,
				"%05d_hud.png",
			);
		} else {
			...
			hudOverlayFinalPath = path.join(
				sequenceRawFilesFolderPath,
				game === Game.CSGO
					? `${sequenceName}%04d_hud.png`
					: `${sequenceName}%08d_hud.png`,
			);
		}
	}

	let hudStartNumber: number | undefined;
	if (hudOverlayActive && hudOverlayType === "url") {
		const hudFiles = await fs.readdir(sequenceRawFilesFolderPath);
		const hudFrameFiles = hudFiles
			.filter((f) => f.endsWith("_hud.png"))
			.sort((a, b) => a.localeCompare(b));
		if (hudFrameFiles.length > 0) {
			const match = hudFrameFiles[0].match(/(\d+)_hud\.png$/);
			if (match) {
				hudStartNumber = Number.parseInt(match[1], 10);
			}
		} else {
			logger.warn(
				`HudCapturer:: No HUD frames found in ${sequenceRawFilesFolderPath}, skipping overlay.`,
			);
		}
	}
	...
	const shouldApplyHud =
		hudOverlayActive &&
		(hudOverlayType === "file" || hudStartNumber !== undefined);
	if (shouldApplyHud) {
		if (hudStartNumber !== undefined) {
			args.push(`-start_number ${hudStartNumber}`);
		}
		args.push(`-i "${hudOverlayFinalPath}"`);
	}
