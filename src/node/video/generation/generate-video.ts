	...
	for (const sequence of sequences) {
		...
		if (encoderSoftware === EncoderSoftware.FFmpeg) {
			await generateVideoWithFFmpeg(
				{
					ffmpegExecutablePath: !isEmptyString(
...
export async function generateVideo(parameters: Parameters) {
	...
	const cleanupFiles = async () => {
		...
		await Promise.all([
			...
			deleteSequencesRawFiles(parameters),
		]);
	};
	...
	try {
		if (hudOverlayEnabled && hudOverlayType === "url") {
			const recordingFolderPath =
				recordingSystem === RecordingSystem.HLAE
					? outputFolderPath
					: await getRecordingFolderPath(game);
			server.sendMessageToMainProcess({
				name: MainServerMessageName.StartHudCapture,
				payload: {
					url: hudOverlayUrl,
					rawFilesFolder: recordingFolderPath,
					width,
					height,
				},
			});
		...
		server.sendMessageToMainProcess({
			name: MainServerMessageName.StopHudCapture,
		});
		await sleep(1000); // Wait for capturer to release files
