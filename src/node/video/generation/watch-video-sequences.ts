...
export async function watchVideoSequences(parameters: Parameters) {
	...
	const { checksum, recordingSystem, demoPath, width, height, game } =
		parameters;
	...
	if (game === Game.CSGO) {
		await createCsgoVideoJsonFile({
			...
			smoothFramerate: parameters.smoothFramerate,
			smoothPlaybackAutoPause: parameters.smoothPlaybackAutoPause,
		});
	} else {
