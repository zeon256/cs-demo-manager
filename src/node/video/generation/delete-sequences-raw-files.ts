...
async function deleteHlaeRawFiles(
	sequenceName: string,
...
async function deleteCs2RawFiles(csgoFolderPath: string) {
	const csgoMovieFolderPath = path.join(
		csgoFolderPath,
		"game",
		"csgo",
		"movie",
	);
	const pluginMovieFolderPath = path.join(
		csgoFolderPath,
		"game",
		"csgo",
		"csdm",
	...
	const pluginMovieFolderExists = await fs.pathExists(pluginMovieFolderPath);
	if (pluginMovieFolderExists) {
		const allFiles = await glob("**/*", {
			cwd: pluginMovieFolderPath,
			absolute: true,
			onlyFiles: true,
		});
		await Promise.all(allFiles.map((f) => fs.remove(f).catch(() => {})));
		// Then try to remove the empty directories
		await fs.remove(pluginMovieFolderPath).catch(() => {});
	}

	const csgoMovieFolderExists = await fs.pathExists(csgoMovieFolderPath);
	if (csgoMovieFolderExists) {
		const allCsgoFiles = await glob("**/*", {
			cwd: csgoMovieFolderPath,
			absolute: true,
			onlyFiles: true,
		});
		await Promise.all(allCsgoFiles.map((f) => fs.remove(f).catch(() => {})));
		await fs.remove(csgoMovieFolderPath).catch(() => {});
	}
}
 ...
 */
async function deleteCsgoRawFiles(
	csgoFolderPath: string,
	sequenceName: string,
) {
	const rawFilesFolderPath = path.join(csgoFolderPath, "csgo");
	const tgaFiles = await glob(["*.tga", "*_hud.png"], {
