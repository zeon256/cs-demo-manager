...
export async function createCsgoVideoJsonFile({
	...
	if (!isSmooth) {
		mandatoryCommands.push("mirv_gameoverlay enable 0", "host_timescale 0");
	}
	...
	for (let i = 0; i < sequences.length; i++) {
		...
		for (const playerOptions of sequence.playersOptions) {
			const replacePlayerNameCommand =
				buildReplacePlayerNameCommand(playerOptions);
		...
		if (type === "record") {
			if (recordingSystem === RecordingSystem.HLAE) {
				json
					.addExecCommand(
						sequence.startTick,
						"mirv_streams add normal defaultNormal",
					)
