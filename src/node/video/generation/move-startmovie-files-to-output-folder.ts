...
export async function moveStartMovieFilesToOutputFolder({
	sequences,
	outputFolderPath,
	game,
}: Options) {
	for (const sequence of sequences) {
		...
		const { tgaFiles, wavFilePath } = await getStartMovieRawFiles({
			sequence,
			game,
		});
		...
		const hudFrames = tgaFiles.map((f) => f.replace(/\.tga$/i, "_hud.png"));
		const existingHudFrames: string[] = [];
		for (const frame of hudFrames) {
			if (await fs.pathExists(frame)) {
				existingHudFrames.push(frame);
			}
		}
		if (existingHudFrames.length > 0) {
			await moveFiles(existingHudFrames, destinationPath);
		}

		if (wavFilePath) {
			await fs.move(
				wavFilePath,
				path.join(destinationPath, `${sequenceName}.wav`),
			);
		}
	}
}
