...
export async function createCs2VideoJsonFile({
	...
	for (let i = 0; i < sequences.length; i++) {
		...
		const setupSequenceTick =
			sequence.startTick - roundedTickrate > 0
				? sequence.startTick - roundedTickrate
				: 1;

		const hlaeOutputFolderPath = getHlaeOutputFolderPath(
			outputFolderPath,
			sequence,
		);
		const presetName =
		...
		json
			.addExecCommand(
				setupSequenceTick,
				`mirv_streams record startMovieWav ${sequence.recordAudio ? 1 : 0}`,
			)
			.addExecCommand(
				setupSequenceTick,
				`mirv_streams record name "${hlaeOutputFolderPath}"`,
			)
			.addExecCommand(setupSequenceTick, "mirv_deathmsg clear")
			.addExecCommand(
				setupSequenceTick,
				`spec_show_xray ${sequence.showXRay ? 1 : 0}`,
			)
			.addExecCommand(
				setupSequenceTick,
				`mp_display_kill_assists ${sequence.showAssists ? 1 : 0}`,
			);
		...
		if (presetName !== "afxClassic") {
			...
			json
				...
				.addExecCommand(
					setupSequenceTick,
					`mirv_streams record screen settings ${presetName}`,
				);
		}
		...
		} else if (smoothFramerate > 0) {
			json.addExecCommand(setupSequenceTick, `fps_max ${smoothFramerate}`);
		}
		...
		if (!isSmooth || smoothPlaybackAutoPause) {
		...
		for (const camera of sequence.playerCameras) {
			const player = players.find(
				(player) => player.steamId === camera.playerSteamId,
			);
			if (player) {
		...
		for (const playerOptions of sequence.playersOptions) {
			...
			if (playerOptions.isVoiceEnabled) {
				const playersWithVoiceEnabled = sequence.playersOptions.filter(
					(playerOptions) => playerOptions.isVoiceEnabled,
				);
				if (playersWithVoiceEnabled.length !== sequence.playersOptions.length) {
					const userIds: number[] = [];
					for (const playerOptions of playersWithVoiceEnabled) {
						const player = players.find(
							(player) => player.steamId === playerOptions.steamId,
						);
						if (player) {
		...
		if (type === "record") {
			if (recordingSystem === RecordingSystem.HLAE) {
				...
				json
					.addExecCommand(
						sequence.startTick,
						`startmovie ${getSequenceName(sequence)}`,
					)
