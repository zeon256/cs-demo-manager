...
import {
	onStartHudCapture,
	onStopHudCapture,
} from "./listeners/on-hud-capture";

export function createWebSocketClient() {
	const client = new WebSocketClient();
	client.on(SharedServerMessageName.NewBannedAccounts, onNewBannedAccounts);
	client.on(
		SharedServerMessageName.NewBannedAccountsError,
		onCheckForNewBannedAccountsError,
	);
	client.on(
		MainServerMessageName.DownloadValveDemoStarted,
		onDownloadValveDemoStarted,
	);
	client.on(
		MainServerMessageName.DownloadFaceitDemoStarted,
		onDownloadFaceitDemoStarted,
	);
	client.on(
		MainServerMessageName.Download5EPlayDemoStarted,
		onDownload5EplayDemoStarted,
	);
	client.on(
		MainServerMessageName.DownloadRenownDemosStarted,
		onDownloadRenownDemosStarted,
	);
	client.on(MainServerMessageName.StartHudCapture, onStartHudCapture);
	client.on(MainServerMessageName.StopHudCapture, onStopHudCapture);
