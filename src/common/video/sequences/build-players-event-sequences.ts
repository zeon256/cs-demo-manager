...
type PlayerEventType =
	| typeof PlayerSequenceEvent.Kills
	| typeof PlayerSequenceEvent.Deaths;
...
export function buildPlayersEventSequences({
	...
	const ticksRequiredBetweenTwoSequences = Math.round(
		match.tickrate * minimumSecondsBetweenTwoEvents,
	);
	const additionalTicksBeforeEvent = Math.round(
		match.tickrate * startSecondsBeforeEvent,
	);
	const additionalTicksAfterEvent = Math.round(
		match.tickrate * endSecondsAfterEvent,
	);
	const maxTicksBetweenEvents = Math.round(
		match.tickrate * maxSecondsBetweenEvents,
	);

	for (const [index, kill] of playerEvents.entries()) {
		const steamIdToFocus = getSteamIdToFocus(
			kill,
			steamIdKey,
			event,
			perspective,
		);
		const sequenceStartTick = Math.max(
			1,
			kill.tick - additionalTicksBeforeEvent,
		);
		let sequenceEndTick = Math.min(
			match.tickCount,
			kill.tick + additionalTicksAfterEvent,
		);
		const nextKill =
			index < playerEvents.length - 1 ? playerEvents[index + 1] : undefined;
		if (nextKill !== undefined) {
			const isNextKillTooClose =
				kill.tick + maxTicksBetweenEvents >= nextKill.tick;
			if (isNextKillTooClose) {
				sequenceEndTick = Math.min(
					match.tickCount,
					nextKill.tick + additionalTicksAfterEvent,
				);
			}
		}

		const previousSequence =
			sequences.length > 0 ? lastArrayItem(sequences) : undefined;
		if (previousSequence !== undefined) {
			const areSequencesOverlapping =
				previousSequence.endTick + ticksRequiredBetweenTwoSequences >=
				sequenceStartTick;
			if (areSequencesOverlapping) {
				...
				const cameraFocusSteamId = getSteamIdToFocus(
					kill,
					steamIdKey,
					event,
					perspective,
				...
				previousSequence.playerCameras.push({
					...
					playerName:
						match.players.find(
							(player) => player.steamId === cameraFocusSteamId,
						)?.name ?? "",
				});
				continue;
			}
			...
			playerCameras: [
				{
					...
					playerName:
						match.players.find((player) => player.steamId === steamIdToFocus)
							?.name ?? "",
				},
			],
			cameras: [],
