...
5.  **Camera & Audio Control**:
    ...
    -   **Multi-kill Sequence Camera Persistence**: When **any** player (main or teammate) is in an active multi-kill sequence, the camera will NOT switch away to other players, even if they get kills during the sequence. The system detects an active sequence by looking both **backward** (recent kills within 10 seconds) AND **forward** (upcoming kills within 10 seconds). If a player has at least 1 recent kill AND at least 1 upcoming kill, they're considered "in a streak" and the camera stays locked on them. Exception: If a main player gets a kill at the exact same tick as a teammate's active sequence, the main player takes priority. This prevents jarring switches during clutch moments - for example, if Teammate B is getting 4 kills and Teammate C gets a single kill in the middle of B's sequence, the camera will stay on B.
